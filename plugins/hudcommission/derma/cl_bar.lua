...
function PANEL:Think()
	...
	for _, v in ipairs(self.bars) do
		...
		-- Permanently keep certain bars visible, as defined above in BAR_PERMASHOW_WHITELIST
		if !BAR_PERMASHOW_WHITELIST[info.identifier] and (v:GetLifetime() < curTime and !info.visible and !bAlwaysShow and !hook.Run("ShouldBarDraw", info)) then
			v:SetVisible(false)
			continue
		end

		if info.color != v:GetColor() then
			v:SetColor(info.color)
		end
...
function PANEL:Init()
	...
	self.bar.Paint = function(this, width, height)
		...
		surface.DrawRect(BAR_GLOW + 1, BAR_GLOW - 1, width - (BAR_GLOW * 2), height - (BAR_GLOW * 2) + 3) -- textured
	end
